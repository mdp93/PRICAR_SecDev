...
def translate_dbc():
    ...
    if response_dbc["status"] != "Found":
        ...
        folder = os.path.join(os.getcwd(),
                                'Downloads', username + '-' + app_id)
        zip_filename = os.path.basename(file.filename)
        zip_file_path = os.path.join(folder, zip_filename)
        filename = zip_filename.replace('.zip','')
        if not os.path.exists(os.path.join(os.getcwd(), 'Downloads')):
            os.makedirs(os.path.join(os.getcwd(), 'Downloads'))
        
        if not os.path.exists(os.path.join(os.getcwd(), 'Downloads', username + '-' + app_id)):
            os.makedirs(os.path.join(os.getcwd(), 'Downloads', username + '-' + app_id))

        if fine_grained_timing:
            ts = time()
        file.save(zip_file_path)
        unzip_directory = os.path.join(folder,filename+'-unzipped')
        os.makedirs(unzip_directory)
        unzip(zip_file_path, unzip_directory)
        if fine_grained_timing:
            unzip_save_time = time() - ts

        # sub_dir = os.path.join(folder, file.filename.replace('.zip', ''))
        sub_dir = unzip_directory
