...
def generate_dbc():
    ...
    :param:     make (str)
                ...
                year (str)
                wheelbase (str)
                zip_file (zipfile): contains all necessary file to run LibreCAN
    ...
    proc = mp.Process(target=LibreCAN.LibreCAN.pipeline.run_pipe,
                      args=(run_name, run_folder, params,))
    proc.daemon = False
    proc.start()
    return flask.jsonify({"run_status": "Processing", "url": flask.request.path})
