...
def send_tcp_msg(dest_host, dest_port, q, tcp_conn_time_start):
    ...
    while not connected:
        try:
            ...
            logging.info("connect failed. try again")
            time.sleep(10)
            continue
        connected = True
    done_sending = False
    ...
    while not done_sending:
        ...
        if messages and messages[0]:
            ...
            #delimit every message with \n
            sending_sock.sendall(str(json.dumps(messages)+'\t').encode('utf-8'))
    sending_sock.close()
    time_end = time.time()
...
def listen_tcp_msg(app):
    ...
    with app:
        ...
        while True:
            # Set up a connection
            #logging.info(f'listening for tcp connections')
            try:
            ...
            while True:
                ...
                if not app_id and not vehicle_str:
                    ...
                    print(f"Connection for app {app_id} for {vehicle_str}")
                    ...
                    #it aint this
                    processing_host, processing_port = endpoint_url.split(":")

                    app_script_dir = os.path.join(openivn.app.config['SCRIPT_DIRECTORY'], str(app_id))
                    #if appID has a script then get CC host and port
                    if os.path.isdir(app_script_dir) :
                        logging.info("getting cc host and port.")
                        filename = os.listdir(app_script_dir)[0]
                        logging.info("hello" + filename)
                        url = openivn.app.config['CLOUD_COMPUTING_API_ENDPOINT']
                        body = {'app_id': app_id}
                        #files = [('file', (filename, open(os.path.join(app_script_dir, filename),'rb'), 'text/plain'))]
                        files = {'file': open(os.path.join(app_script_dir, filename),'rb')}
                        logging.info('got here 0')
                        #attach any input files
                        app_script_input_dir = os.path.join(openivn.app.config['SCRIPT_DIRECTORY'], str(app_id) + '-inputs')
                        logging.info("got here")
                        if os.path.isdir(app_script_input_dir):
                            i=0
                            logging.info("got here1")
                            for input_filename in os.listdir(app_script_input_dir):
                                files['file_'+str(i)] = open(os.path.join(app_script_input_dir, input_filename), 'rb')
                                #files.append(('file_'+str(i), (input_filename, open(os.path.join(app_script_input_dir, input_filename), 'rb'), 'text/plain')))
                                i+=1
                        ...
                        processing_host = response['host']
                        processing_port = response['port']
                    #if no script then skip CC host and send to the CC receiver
                    else:
                        processing_host = '0.0.0.0'
                        processing_port = openivn.app.config['CLOUD_COMPUTING_RECEIVER_PORT']
                    logging.info(f"Host: {processing_host}, Port: {processing_port}")
                    # create a MP Queue
                    send_q = mp.Queue()
                    ...
                    sender = mp.Process(target=send_tcp_msg,
                                        args=(processing_host, processing_port, send_q, tcp_conn_time_start),)
                    # start the sender
                    sender.daemon = True
